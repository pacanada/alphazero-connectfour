class RLAgent:
    ...
    def choose_action(self, actions, list_states, player):
        """Choose action from current possible actions """
        if (self.model_is_fitted) and (self.cont % self.ratio_explotation_exploration == 0):
            if player==-1:
...
class RLSimulation:
    ...
    def run(self,):
        ...
        while True:
            ...
            outcome = self.game.play_mcts(action=action)
